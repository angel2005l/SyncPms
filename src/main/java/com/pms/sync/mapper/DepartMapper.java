...
public interface DepartMapper {
	...
	@Select("SELECT COUNT(1) FROM CM_DEPART WHERE  NODE_TYPE = '1' AND  DEPT_CODE = #{deptCode,jdbcType=VARCHAR}")
	public Integer selDepartNumByDeptCode(String deptCode) throws Exception;

	@Select("SELECT DEPT_NO FROM CM_DEPART WHERE NODE_TYPE = '1' AND  DEPT_CODE = #{deptCode,jdbcType=VARCHAR} ")
	public String selDeptNoByDepartCode(String deptCode) throws Exception;

	@Select("INSERT INTO CM_DEPART(WID,DEPT_NO,DEPT_CODE,DEPT_NAME,PARENT_DEPT_NO,DEPT_TYPE,NODE_TYPE,LIMIT_NUM,UPT_FLAG) VALUES(#{wid},#{deptNo},#{deptCode,jdbcType=VARCHAR},#{deptName,jdbcType=VARCHAR},#{deptCode,jdbcType=VARCHAR},'SYS_DEPART','1',0,'Y')")
	public Integer insDepart(Depart depart) throws Exception;

	@Select("UPDATE CM_DEPART SET DEPT_NAME=#{deptName},PARENT_DEPT_NO=#{parentDeptNo,jdbcType=VARCHAR},USER_VP=#{userVp,jdbcType=VARCHAR},UPT_FLAG='Y' where DEPT_CODE =#{deptCode,jdbcType=VARCHAR} AND NODE_TYPE = '1' for update")
	public Integer uptDepart(Map<String, String> depart) throws Exception;

	@Delete("DELETE CM_DEPART WHERE  DEPT_CODE =#{deptCode} AND NODE_TYPE = '1'")
	public Integer delDepart(String deptCode) throws Exception;

	@Select("SELECT * FROM CM_DEPART WHERE  NODE_TYPE = '1' AND  DEPT_NO = #{deptNo,jdbcType=VARCHAR}")
	...
	@Select("SELECT * FROM CM_DEPART WHERE UPT_FLAG = 'Y' AND NODE_TYPE = '1'")
	@ResultMap(value = "deptMap")
	public List<Depart> selDepartWithUpt() throws Exception;

	@Update("UPDATE CM_DEPART SET DEPT_NO_PATH = #{deptNoPath},UPT_FLAG='N' where WID = #{wid} AND NODE_TYPE = '1' for update" )
