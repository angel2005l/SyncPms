...
public interface DepartMapper {
	...
	@Select("SELECT COUNT(1) FROM CM_DEPART_TE WHERE  NODE_TYPE = '1' AND  DEPT_CODE = #{deptCode}")
	public int selDepartNumByDeptCode(String deptCode) throws Exception;

	@Select("SELECT DEPT_NO FROM CM_DEPART_TE WHERE NODE_TYPE = '1' AND  DEPT_CODE = #{deptCode} ")
	public String selDeptNoByDepartCode(String deptCode) throws Exception;

	@Select("INSERT INTO CM_DEPART_TE(WID,DEPT_NO,DEPT_CODE,DEPT_NAME,DEPT_TYPE,NODE_TYPE,LIMIT_NUM,UPT_FLAG) VALUES(#{wid},#{deptNo},#{deptCode},#{deptName},'SYS_DEPART','1',0,'Y')")
	public int insDepart(Depart depart) throws Exception;

	@Select("UPDATE CM_DEPART_TE SET DEPT_NAME=#{deptName},PARENT_DEPT_NO=#{parentDeptNo},USER_VP=#{userVp},UPT_FLAG='Y' where DEPT_CODE =#{deptCode} AND NODE_TYPE = '1'")
	public int uptDepart(Map<String, String> depart) throws Exception;

	@Delete("DELETE CM_DEPART_TE WHERE  DEPT_CODE =#{deptCode} AND NODE_TYPE = '1'")
	public int delDepart(String deptCode) throws Exception;
	...
	public int uptDepartForDeptNoPath(@Param("wid") String wid, @Param("deptNoPath") String deptNoPath)
			throws Exception;
